...
import { MapPin, Droplets, AlertTriangle, Cloud, RefreshCw } from "lucide-react";
...
const FloodMap = () => {
  ...
  const predictWeather = async () => {
    ...
    try {
      ...
      toast.success(`Weather predictions updated for ${data.updated || 0} zones`);
      
      // Refresh zones to show updated predictions
      await fetchZones();
    } catch (error: any) {
      console.error("Error predicting weather:", error);
      toast.error("Failed to predict weather: " + (error.message || "Unknown error"));
    } finally {
      setPredicting(false);
    }
  };
